...
module ActiveRecord
  module ConnectionAdapters
    ...
    class EmPostgreSQLAdapter < PostgreSQLAdapter
      ...
      def initialize(connection, logger, connection_parameters, config)
        @hostname = connection_parameters[0]
        @port = connection_parameters[1]
        @connect_parameters = connection_parameters[4..-1]
      ...
      def connect
        if EM.reactor_running?
          ...
          @logger.debug "[em_postgresql] EM not running, falling back to blocking behavior"
          super
        end
      end
  ...
  class Base
    ...
    def self.em_postgresql_connection(config) # :nodoc:
      ...
      ConnectionAdapters::EmPostgreSQLAdapter.new(nil, logger, [host, port, nil, nil, database, username, password], config)
    end
  end
